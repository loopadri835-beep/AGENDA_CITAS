...
const JohanaMakeupSalon = () => {
  ...
  const handleBooking = async () => {
    if (!selectedService || !selectedSubService || !selectedDate || !selectedTime || !clientName || !clientEmail || !clientPhone) {
                    ...
                    return (
                      <div
                        ...
                        className={`schedule-cell schedule-slot ${
                          isPast ? 'past' : isOccupied ? 'occupied' : '16:41available'
}`}
>
    ...
    {selectedService && (
      <div className="form-group">
        <label className="form-label">
          Tipo de {servicesData[selectedService].name}
        </label>
        <select
...
return currentView === 'home' ? <HomePage /> : currentView === 'schedule' ? <SchedulePage /> : <BookingPage />;
};
